...
def build_markdown(
    ...
    lines += [
        ...
        "",
        f"_Generated by [custom_agent]({_agent_dir_link()}) — ReAct + audit_agent scanners_",
    ]
...
def _agent_dir_link() -> str:
    """Absolute link to the custom_agent/ dir in this repo — a bare relative
    path doesn't reliably resolve when this text is posted as a PR comment
    body rather than rendered as part of a repo file."""
    slug = _github_repo_slug()
    if not slug:
        return "custom_agent/"
    return f"https://github.com/{slug}/tree/{_github_ref()}/custom_agent"
